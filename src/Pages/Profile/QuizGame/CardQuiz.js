...
export default function App() {
  ...
  const handleNextQuestion = () => {
    if (currentQuestionIndex < quiz.questions.length - 1) {
      ...
      setFlippedStates(
        new Array(
          quiz.questions[currentQuestionIndex + 1].solutionCharacters.length
        ).fill(false)
  ...
  const handleSubmit = () => {
    const actualWord = currentQuestion.solutionCharacters
      .join("")
      .toLowerCase();
  ...
  return (
    <div className="app-container">
      ...
      <button
        ...
        disabled={
          currentQuestionIndex === quiz.questions.length - 1 || isQuizComplete
        }
      >
        Next Question
      </button>

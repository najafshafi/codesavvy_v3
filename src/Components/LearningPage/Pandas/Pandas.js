...
import topics from "./Pandas.json";
import "../Web.css";
    ...
    <ul className="topic-list">
      {topics.map((topic, index) => (
        <li
          ...
          style={{
            backgroundColor:
              selectedTopic.title === topic.title ? "black" : "inherit",
...
const PandasTutorial = () => {
  const [selectedTopic, setSelectedTopic] = useState(
    topics.pandas_tutorial[0]
  );
  ...
  return (
    <div className="container">
      <Sidebar
        topics={topics.pandas_tutorial}
        onTopicSelect={handleTopicSelect}
        selectedTopic={selectedTopic}
      />
      <div className="content">
      <h1 className="text-5xl text-black">Pandas</h1>
        <TutorialSection

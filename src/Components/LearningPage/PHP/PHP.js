    ...
    <ul className="topic-list">
      {topics.map((topic, index) => (
        <li
          ...
          style={{
            backgroundColor:
              selectedTopic.title === topic.title ? "#AEB2D5 " : "inherit",
...
const PHPTutorial = () => {
  const [selectedTopic, setSelectedTopic] = useState(topics.php_tutorial[0]);
  ...
  return (
    <div className="container">
      <Sidebar
        topics={topics.php_tutorial}
        onTopicSelect={handleTopicSelect}
        selectedTopic={selectedTopic}
      />
      <div className="content">
      <h1 className="text-5xl text-indigo-900">PHP</h1>
        <TutorialSection
          title={selectedTopic.title}
          content={selectedTopic.content}
          codeExample={selectedTopic.examples}
        />
      </div>
    </div>
  );
